...
mkdir -p "$TMP_ROOT"
VAULT=$(mktemp -d "${TMP_ROOT}/index-test-XXXXXXXX")
...
"${VAULT}/0_core/script/index.sh" --vault "$VAULT" --target "${VAULT}/2_knowledge/research/sqlite-fts.md" >/dev/null
