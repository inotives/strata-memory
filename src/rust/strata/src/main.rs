...
fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::SemanticRefresh => {
            let summary = semantic_refresh(&cli.vault)?;
            if cli.json {
                println!(
                    "{{\"ok\":true,\"provider\":\"{}\",\"model\":\"{}\",\"embedding_dim\":{},\"indexed\":{},\"descriptions\":{},\"sections\":{}}}",
...
fn semantic_status(vault: &Path, json: bool) -> Result<()> {
    ...
    if json {
        println!(
            "{{\"ok\":true,\"semantic_available\":{},\"provider_configured\":{},\"runtime_available\":{},\"vector_index_ready\":{},\"vector_extension_available\":{},\"fallback\":\"{}\",\"provider\":\"{}\",\"model\":\"{}\",\"embedding_dim\":{},\"embedding_count\":{},\"model_count\":{}}}",
            semantic_available,
            provider_configured,
            runtime_available,
            vector_index_ready,
...
fn semantic_refresh(vault: &Path) -> Result<SemanticRefreshSummary> {
    db::migrate(vault)?;
    let semantic = config::semantic(vault)?;
