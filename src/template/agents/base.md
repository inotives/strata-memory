...
- Use `strata promote` only for a new durable document, with `--approved-by ACTOR`; it creates the durable file and permanently removes the draft. To revise an already-promoted document, edit its existing durable file directly, preserve its `id`, increment `version`, and update `modified`, `modified_by`, and `last_edit_summary` when present. Permanently delete unwanted files, then run `strata refresh`. Treat any `_archived/` content as legacy and ignored.
